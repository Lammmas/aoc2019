...
def move(direction, position = [0, 0]):
	...
	if direction == "U":
		...
		print("Unknown direction for move: " + direction)
...
def mapRoute(steps):
	...
	for idx in range(len(steps)):
		...
		for s in range(length):
			position = move(direction, copy.deepcopy(position))
			route.append(position)
	return route

def centerDistance(coords = [0, 0]):
	return abs(coords[0]) + abs(coords[1])

routeOne = mapRoute(wireOne)
routeTwo = mapRoute(wireTwo)

closestDistance = -1
count = len(routeOne) * len(routeTwo)

for x in range(len(routeOne)):
	for y in range(len(routeTwo)):
		if (routeOne[x][0] == routeTwo[y][0]) and (routeOne[x][1] == routeTwo[y][1]):
			dist = centerDistance(routeOne[x])

			if closestDistance == -1 or closestDistance > dist:
				closestDistance = dist
		sys.stdout.write("\r%i/%i" % (x * 1, count))
		sys.stdout.flush()

print("Solution: " + str(closestDistance))