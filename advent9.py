...
def executeCommand(commands = [], idx = 0, inVal = 0, state = { "relBase": 0 }):
	...
	if code == 99:
		return commands, idx, state, None, True
	...
	firstVal = getModeVal(firstIdx, modes[0])

	if opCode == 9:
		state["relBase"] += firstVal
		idx += 2
		return commands, idx, state, None, False
	if opCode == 4:
		idx += 2
		return commands, idx, state, firstVal, False
	if opCode == 3:
		# commands[firstIdx] = inVal
		storeModeVal(firstIdx, inVal, modes[0])
		idx += 2
		return commands, idx, state, None, False

	secondIdx = commands[idx + 2]
	secondVal = getModeVal(secondIdx, modes[1])
	...
	if opCode == 8:
		storeModeVal(outIdx, 1 if firstVal == secondVal else 0, modes[2])
	if opCode == 7:
		storeModeVal(outIdx, 1 if firstVal < secondVal else 0, modes[2])
	elif opCode == 2:
		storeModeVal(outIdx, firstVal * secondVal, modes[2])
	elif opCode == 1:
		storeModeVal(outIdx, firstVal + secondVal, modes[2])
...
def compute(commands = [], inp = 0):
	...
	while position < len(commands):
		commands, position, state, out, stop = executeCommand(commands, position, inp, state)
